...
use crate::server::backend_state::{AttributeChange, EditAttribute, GlobalState, Task};
...
pub fn backend_thread(channel: std::sync::mpsc::Receiver<Task>, backend: Arc<GlobalState>, tokio_handle: tokio::runtime::Handle) {
    while let Ok(task) = channel.recv() {
        match task {
            Task::SendNotification(notification) => send_notification(&backend, notification, &tokio_handle),
            Task::EditAttribute(edit) => edit_attribute(&backend, edit),
            Task::EditOnlineState(causing_component, element_id, state) => edit_online_state(&backend, causing_component, element_id, state),
            Task::Reconfigure => run_reconfiguration(&backend),
        }
    }
}
fn send_notification(backend: &Arc<GlobalState>, notification: Notification, handle: &tokio::runtime::Handle) {
...
fn edit_attribute(backend: &Arc<GlobalState>, edit: EditAttribute) {
    let change = match edit.change {
        AttributeChange::Set(new) => {
            ...
            let element = if let Some(element) = state_lock.get_mut(&edit.element_id) { element } else {
                ...
                backend.enqueue(Task::SendNotification(Notification {
        ...
        AttributeChange::Delete => {
            ...
            AttributeValueChange::Delete(old_value)
        }
    };
    backend.enqueue(Task::SendNotification(Notification {
        component_id: edit.causing_component.to_string(),
...
fn edit_online_state(backend: &Arc<GlobalState>, causing_component: &'static str, element_id: String, new_state: bool) {
    let mut state_lock = backend.states.write();
    let reason = if let Some(element) = state_lock.get_mut(&element_id) {
        if element.online == new_state { return; }
        element.online = new_state;
        NotificationReason::OnlineStatusChanged(new_state)
    } else {
        assert!(state_lock.insert(element_id.clone(), crate::State::with_online(new_state)).is_none());
        NotificationReason::NewElement(new_state)
    };
...
fn run_reconfiguration(backend: &Arc<GlobalState>) {
